...
def dispatch_emails(execute=False, approval_file=DEFAULT_APPROVAL_FILE,
                    dnc_file=DEFAULT_DNC_FILE, ledger_file=DEFAULT_LEDGER_FILE):
    audit_file = Path("audit_results.json")
    if not audit_file.exists():
        print("[ERROR] audit_results.json not found. Run batch_audit_scanner.py first.")
        return False
    ...
    if execute and (not smtp_user or not smtp_password):
        print("[BLOCKED] Live execution requested but SMTP credentials are unavailable.")
        return False
    ...
    for audit in audits:
        ...
        proposal_file = Path("proposals") / f"{domain.replace('.', '_')}_proposal.txt"
    ...
    print(f"[DONE] sent={sent_count} blocked={blocked_count} execute={execute}")
...
def main():
    ...
    raise SystemExit(0 if dispatch_emails(args.execute, args.approval_file, args.dnc_file, args.ledger_file) else 1)
