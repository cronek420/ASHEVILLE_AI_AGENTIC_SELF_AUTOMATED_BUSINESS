...
if __name__ == "__main__":
    ...
    parser.add_argument("targets", nargs="*", help="Optional manual targets")
    ...
    if not targets:
        ...
        leads_file = f"scraped_leads_{args.tenant}.json"
        if os.path.exists(leads_file):
            with open(leads_file, "r") as f:
                scraped = json.load(f)
            if scraped:
                targets = [s["domain"] for s in scraped]
                print(f"Loaded {len(targets)} targets from {leads_file}")

        if not targets:
            print(f"{leads_file} empty or missing. Using {len(ASHEVILLE_TARGETS)} hardcoded targets.")
            targets = ASHEVILLE_TARGETS

    run_id = f"RUN-{datetime.datetime.now().strftime('%Y%m%d-%H%M')}-{args.tenant.upper()}"
    results = batch_audit(targets, run_id=run_id)

    # Save JSON results
    output_file = f"audit_results_{args.tenant}.json"
