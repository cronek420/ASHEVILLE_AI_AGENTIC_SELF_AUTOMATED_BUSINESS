...
# Written next to a proposal whose payment links could not all be resolved. The
# dispatcher refuses to send any proposal that has one, and live_sheets_sync
# turns the prospect's row red so it is obvious in the Command Center.
REVIEW_MARKER = "{domain}_NEEDS_REVIEW.json"
...
def _spreadsheet_id(tenant):
    import yaml

    with open("tenants.yaml", "r", encoding="utf-8") as handle:
        tenants = (yaml.safe_load(handle) or {}).get("tenants", {}) or {}
    return (tenants.get(tenant) or {}).get("spreadsheet_id", "")


def flagged_domains(tenant):
    """Domains whose proposal is marked NEEDS REVIEW and must not be sent."""
    out_dir = os.path.join(os.path.dirname(__file__), f"proposals_{tenant}")
    if not os.path.isdir(out_dir):
        return set()
    found = set()
    for name in os.listdir(out_dir):
        if name.endswith("_NEEDS_REVIEW.json"):
            try:
                with open(os.path.join(out_dir, name), "r", encoding="utf-8") as handle:
                    found.add(json.load(handle).get("domain", ""))
...
def generate_proposals(tenant):
    ...
    # The city sells the offer Tom selected in 'Start Here'. One city, one offer.
    # Refuse rather than default: quoting an offer nobody chose is the failure
    # this replaced.
    try:
        product_id, active_product, offer_note = active_offer.resolve(
            tenant, _spreadsheet_id(tenant), PRODUCTS)
    except ValueError as exc:
        print(f"[BLOCKED] {exc}")
        return
    print(f"[OFFER] {tenant} is selling '{active_product['name']}' "
          f"at ${active_product['total']}.")
    if offer_note:
        print(f"[OFFER] {offer_note}")
    ...
    for a in audits:
        if a.get("status") == "AUDITED":
            ...
            stripe_deposit = STRIPE_DEPOSIT_LINKS.get(prod["deposit"])
            stripe_full = STRIPE_FULL_LINKS.get(prod["total"])
            stripe_balance = STRIPE_BALANCE_LINKS.get(balance)

            # A dead payment link must never reach a real business. The proposal
            # is still written so it can be edited, but it is marked and the
            # dispatcher will not send it.
            missing = [label for label, link in (
                (f"deposit ${prod['deposit']}", stripe_deposit),
                (f"full ${prod['total']}", stripe_full),
                (f"balance ${balance}", stripe_balance),
            ) if not link]
            stripe_deposit = stripe_deposit or "[NO PAYMENT LINK CONFIGURED]"
            stripe_full = stripe_full or "[NO PAYMENT LINK CONFIGURED]"
            stripe_balance = stripe_balance or "[NO PAYMENT LINK CONFIGURED]"
            ...
            marker_path = os.path.join(out_dir, REVIEW_MARKER.format(domain=safe_domain))

            if missing:
                banner = (
                    "!! NEEDS REVIEW — DO NOT SEND AS-IS !!\n"
                    f"No Stripe payment link is configured for: {', '.join(missing)}.\n"
                    "Add the link in proposal_engine.py, then re-run this step.\n"
                    + "=" * 70 + "\n\n"
                )
                proposal_text = banner + proposal_text
                with open(marker_path, "w", encoding="utf-8") as handle:
                    json.dump({
                        "domain": domain,
                        "reason": f"missing payment link(s): {', '.join(missing)}",
                        "offer": prod["name"],
                ...
                # The gap was fixed; clear the old flag so the row stops being red.
                os.remove(marker_path)

            out_file = os.path.join(out_dir, f"{safe_domain}_proposal.txt")
            with open(out_file, "w", encoding="utf-8") as f:
                f.write(proposal_text)

            generated += 1

    print(f"[SUCCESS] Generated {generated} proposal(s) for {tenant}, "
          f"all quoting '{active_product['name']}' at ${active_product['total']}.")
    if flagged:
        print(f"[NEEDS REVIEW] {len(flagged)} proposal(s) have no usable payment link "
              f"and will NOT be sent: {', '.join(sorted(flagged)[:5])}"
