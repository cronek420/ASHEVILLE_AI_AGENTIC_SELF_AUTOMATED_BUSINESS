...
- **Specialist Roles:** Cover the functions of `Atlas-Orchestrator`, `Scout-Research`, `Offer-Strategist`, `Proof-Builder`, `Outreach-Drafter`, `Sales-Coordinator`, `Delivery-Builder`, `QA-Risk`, and `Finance-Recorder`. These are **roles, not running processes** — one session performs them in sequence. The authoritative roster of programs that actually execute is `.agents/agents.md`; keep the two consistent.
